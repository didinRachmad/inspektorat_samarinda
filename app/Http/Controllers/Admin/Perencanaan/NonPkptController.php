...
class NonPkptController extends Controller
{
    public function index()
    {
        ...
        $jenisPengawasans = JenisPengawasan::with(['children' => fn($q) => $q->orderBy('urutan')])
            ...
            ->orderBy('urutan')
            ->get();
        return view('perencanaan.non_pkpt.index', compact('mandatories', 'auditis', 'irbanwils', 'jenisPengawasans'));
    }
    ...
    public function data()
    {
        ...
        return DataTables::of($query)
            ...
            ->addColumn('edit_url', fn($row) => route('non_pkpt.edit', $row->id))
            ->addColumn('delete_url', fn($row) => route('non_pkpt.destroy', $row->id))
    ...
    public function store(StorePkptRequest $request)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('non_pkpt.index')->with('success', 'Data PKPT berhasil dibuat.');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function update(UpdatePkptRequest $request, Pkpt $pkpt)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('non_pkpt.index')->with('success', 'Data PKPT berhasil diperbarui.');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function destroy(Pkpt $pkpt)
    {
        ...
        try {
            ...
            $pkpt->delete();
            DB::commit();
            return redirect()->route('non_pkpt.index')->with('success', 'Data PKPT berhasil dihapus.');
        } catch (\Exception $e) {
            DB::rollBack();
