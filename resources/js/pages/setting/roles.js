...
const routes = {
    datatable: () => route("roles.data"),
};

class RolesPage {
    constructor() {
        this.pageName = "Setting Roles";
    ...
    initEdit() {
        ...
        this.initPermissionToggle();
    }

    initPermissionToggle() {
        // Fungsi global agar bisa dipanggil dari event inline di Blade
        window.toggleMenuPermissionsCheckbox = (masterCheckbox, menuId) => {
                ...
                { data: "name", name: "name" },
                {
                    ...
                    render: (data, type, row) => {
                        ...
                        if (row.can_edit_permission) {
                            buttons += `
                                <a href="${row.edit_permission_url}" class="btn btn-sm btn-info rounded-4" data-bs-toggle="tooltip" title="Atur Permissions">
