...
@section('dashboard-content')
    ...
    <div class="card rounded-4 w-100 m-0">
        <form action="{{ route('kode_rekomendasi.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="card-header">
                <h5 class="card-title" id="addModalLabel">Tambah Data</h5>
            </div>
            <div class="card-body">
                @include('master.kode_rekomendasi._form')
            </div>
            <div class="card-footer d-flex justify-content-between align-items-center">
                <a href="{{ route('kode_rekomendasi.index') }}" class="btn btn-sm rounded-4 btn-secondary">Batal <i
